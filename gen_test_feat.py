...
t_click_file = 'D:/Jupyter/2017JDD-Loan_Forecasting_Qualification-master/data/t_click.csv'
t_loan_sum_file = 'D:/Jupyter/2017JDD-Loan_Forecasting_Qualification-master/data/t_loan_sum.csv'
t_loan_file = 'D:/Jupyter/2017JDD-Loan_Forecasting_Qualification-master/data/t_loan.csv'
t_order_file = 'D:/Jupyter/2017JDD-Loan_Forecasting_Qualification-master/data/t_order.csv'
t_user_file = 'D:/Jupyter/2017JDD-Loan_Forecasting_Qualification-master/data/t_user.csv'
...
def gen_loan_feat():
    ...
    if os.path.exists(dump_path):
        ...
        statistic_df = statistic_df.groupby(['uid', 'month'], as_index=False).sum(numeric_only=True).reset_index()
        ...
        exceed_df = exceed_df.groupby(['uid', 'month'], as_index=False).sum(numeric_only=True).reset_index()
        ...
        # 尝试使用列名访问（假设第8列是'loan_amount'，请根据你的csv内容调整）
        # 或者使用 .iloc 按位置访问
        # 访问 8 月份那一列的借款金额来减去 limit
        exceed_df['exceed_loan_1'] = exceed_df.get('8', 0) - exceed_df['limit']
        exceed_df['exceed_loan_2'] = exceed_df.get('9', 0) - exceed_df['limit']
        exceed_df['exceed_loan_3'] = exceed_df.get('10', 0) - exceed_df['limit']
        ...
        df_loan = df_loan.groupby(['uid', 'month'], as_index=False).sum(numeric_only=True)
